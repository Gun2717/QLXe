    ...
    private List<Map<String, Object>> filterOrdersByTimeRange(List<Map<String, Object>> orders, String timeRange) {
        ...
        for (Map<String, Object> order : orders) {
            try {
                ...
                // Parse date from format "2025-11-24T10:00:00"
                Date orderDate;
                if (createdAt.length() >= 10) {
                    String datePart = createdAt.substring(0, 10);
                    orderDate = new SimpleDateFormat("yyyy-MM-dd").parse(datePart);
                } else {
                    continue;
                }
                ...
                switch (timeRange) {
                    case "7 ngày gần nhất":
                        ...
                        include = orderDate.after(cal.getTime()) || orderDate.equals(cal.getTime());
                        break;
                    case "Tháng này":
                        cal.setTime(now);
