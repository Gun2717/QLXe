...
public class InventoryPanel extends JPanel {
    ...
    // Statistics labels
    private JLabel lblTotalProductsStat;
    private JLabel lblTotalStockStat;
    private JLabel lblLowStockStat;
    ...
    private void initComponents() {
        ...
        lblTotalProductsStat = new JLabel("Tổng sản phẩm: 0");
        lblTotalStockStat = new JLabel("Tổng tồn kho: 0");
        lblLowStockStat = new JLabel("Sắp hết hàng: 0");

        lblTotalProductsStat.setFont(new Font("Arial", Font.PLAIN, 14));
        lblTotalStockStat.setFont(new Font("Arial", Font.PLAIN, 14));
        lblLowStockStat.setFont(new Font("Arial", Font.PLAIN, 14));
        lblLowStockStat.setForeground(new Color(231, 76, 60));

        statsPanel.add(lblTotalProductsStat);
        statsPanel.add(lblTotalStockStat);
        statsPanel.add(lblLowStockStat);
                ...
                try {
                    ...
                    if (response.isSuccess()) {
                        ...
                        for (Map<String, Object> item : inventory) {
                            // Safely get values
                            int id = getIntValue(item.get("id"));
                            String productName = getStringValue(item.get("productName"));
                            String brand = getStringValue(item.get("brand"));
                            String model = getStringValue(item.get("model"));
                            int quantityIn = getIntValue(item.get("quantityIn"));
                            int quantityOut = getIntValue(item.get("quantityOut"));
                            int currentQty = getIntValue(item.get("quantityCurrent"));
                            ...
                            String lastUpdated = "";
                            try {
                                Object lastUpdatedObj = item.get("lastUpdated");
                                if (lastUpdatedObj != null) {
                                    lastUpdated = sdf.format(new java.util.Date(lastUpdatedObj.toString()));
                                }
                            } catch (Exception e) {
                                lastUpdated = "N/A";
                            }
    ...
    private void updateStatistics(int totalProducts, int totalStock, int lowStock) {
        lblTotalProductsStat.setText("Tổng sản phẩm: " + totalProducts);
        lblTotalStockStat.setText("Tổng tồn kho: " + totalStock);
        lblLowStockStat.setText("Sắp hết hàng: " + lowStock);
    }
    ...
    private void showExportDialog() {
        ...
        if (currentStock <= 0) {
            JOptionPane.showMessageDialog(this,
                    "Sản phẩm không còn hàng trong kho!",
                    "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
                ...
                try {
                    ...
                    if (response.isSuccess()) {
                        ...
                        for (Map<String, Object> p : products) {
                            int id = getIntValue(p.get("id"));
                            String name = getStringValue(p.get("name"));
                            String brand = getStringValue(p.get("brand"));
                            String modelStr = getStringValue(p.get("model"));
                            int quantityCurrent = getIntValue(p.get("quantityCurrent"));
                            double price = getDoubleValue(p.get("price"));
