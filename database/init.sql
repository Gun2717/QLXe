...
-- Script thêm dữ liệu đơn hàng hôm nay để test Dashboard
USE motorcycle_shop;

-- Xóa đơn hàng hôm nay (nếu có) để tránh trùng
DELETE FROM order_items WHERE order_id IN (
    SELECT id FROM orders WHERE DATE(created_at) = CURDATE()
);
DELETE FROM orders WHERE DATE(created_at) = CURDATE();

-- Thêm đơn hàng hôm nay
INSERT INTO orders (order_code, customer_id, user_id, total_amount, discount_amount, final_amount, status, payment_method, notes, created_at) VALUES
(CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '001'), 1, 3, 47990000, 0, 47990000, 'COMPLETED', 'CASH', 'Đơn hàng hôm nay 1', NOW()),
(CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '002'), 2, 3, 41490000, 1000000, 40490000, 'COMPLETED', 'TRANSFER', 'Đơn hàng hôm nay 2', NOW()),
(CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '003'), 3, 4, 155000000, 5000000, 150000000, 'COMPLETED', 'CARD', 'Đơn hàng hôm nay 3', NOW()),
(CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '004'), 4, 3, 52990000, 0, 52990000, 'COMPLETED', 'CASH', 'Đơn hàng hôm nay 4', NOW()),
(CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '005'), 5, 4, 29990000, 500000, 29490000, 'PENDING', 'CASH', 'Đơn đang xử lý', NOW());

-- Thêm chi tiết đơn hàng
SET @order1 = (SELECT id FROM orders WHERE order_code = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '001'));
SET @order2 = (SELECT id FROM orders WHERE order_code = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '002'));
SET @order3 = (SELECT id FROM orders WHERE order_code = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '003'));
SET @order4 = (SELECT id FROM orders WHERE order_code = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '004'));
SET @order5 = (SELECT id FROM orders WHERE order_code = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d'), '005'));

INSERT INTO order_items (order_id, product_id, quantity, unit_price, total_price) VALUES
(@order1, 5, 1, 47990000, 47990000),
(@order2, 8, 1, 41490000, 41490000),
(@order3, 10, 1, 155000000, 155000000),
(@order4, 16, 1, 52990000, 52990000),
(@order5, 7, 1, 29990000, 29990000);

-- Cập nhật một số sản phẩm có số lượng thấp để test chức năng "Sắp hết hàng"
UPDATE inventory SET quantity_current = 3 WHERE product_id IN (1, 2);
UPDATE inventory SET quantity_current = 7 WHERE product_id IN (3, 4);
UPDATE inventory SET quantity_current = 9 WHERE product_id IN (5, 6);

UPDATE products SET quantity = 3 WHERE id IN (1, 2);
UPDATE products SET quantity = 7 WHERE id IN (3, 4);
UPDATE products SET quantity = 9 WHERE id IN (5, 6);

-- Kiểm tra kết quả
SELECT
    'Đơn hàng hôm nay' as Loai,
    COUNT(*) as SoLuong,
    SUM(final_amount) as TongTien
FROM orders
...
SELECT
    ...
    NULL
FROM inventory
WHERE quantity_current < 10;

-- Hiển thị chi tiết đơn hàng hôm nay
SELECT
    o.order_code as MaDon,
    c.full_name as KhachHang,
    u.full_name as NhanVien,
    FORMAT(o.final_amount, 0) as ThanhTien,
    o.status as TrangThai,
    DATE_FORMAT(o.created_at, '%H:%i:%s') as GioTao
FROM orders o
LEFT JOIN customers c ON o.customer_id = c.id
LEFT JOIN users u ON o.user_id = u.id
...
SELECT
    ...
    p.name as TenSP,
    p.brand as Hang,
    i.quantity_current as TonKho,
    CASE
        WHEN i.quantity_current < 5 THEN 'Cần nhập ngay'
        WHEN i.quantity_current < 10 THEN 'Sắp hết'
        ELSE 'Đủ hàng'
    END as TrangThai
FROM products p
LEFT JOIN inventory i ON p.id = i.product_id
WHERE i.quantity_current < 10
ORDER BY i.quantity_current ASC;

SELECT '✅ Đã thêm dữ liệu test thành công!' as KetQua;